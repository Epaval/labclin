...
class RecordatoriosWhatsAppView(LoginRequiredMixin, TemplateView):
    """Examenes/Resultados del dia con boton de notificacion WhatsApp."""
    ...
    def get_context_data(self, **kwargs):
        ...
        # resultados publicados del dia (aptos para notificar)
        try:
            from apps.results.models import Resultado
            ctx["resultados"] = (Resultado.objects.filter(fecha__date=f, estado="publicado")
...
class ConfiguracionWhatsAppView(LoginRequiredMixin, View):
    def get(self, request):
        from apps.core.models import ConfiguracionClinica
        from apps.core.whatsapp import renderizar_plantilla
        cfg = ConfiguracionClinica.cargar()
        preview = renderizar_plantilla(cfg.plantilla_whatsapp, nombre="Maria Perez",
                                       clinica=cfg.nombre_clinica, servicio="Hemograma",
